...
# Example: "Box 1: 3-1-0  Track 15: 4-3-2  Distance 10: 2-1-1"
RE_BOX   = re.compile(r"Box\s*(\d+):\s*(\d+)-(\d+)-(\d+)", re.IGNORECASE)
RE_TRACK = re.compile(r"Track\s*(\d+):\s*(\d+)-(\d+)-(\d+)", re.IGNORECASE)
RE_DIST  = re.compile(r"Distance\s*(\d+):\s*(\d+)-(\d+)-(\d+)", re.IGNORECASE)
...
def parse_box_history(text: str, box_bias_map: Optional[dict] = None) -> pd.DataFrame:
    ...
    for line in lines:
        if "Box" not in line:
            continue
        if ("Track" in line or "Distance" in line):
            r: Dict[str, Any] = {
                "BoxHistory": line,
                "Box": None, "BoxWins": None, "BoxPlaces": None, "BoxBiasFactor": None,

...
def load(date_from='2017-12-01', date_to='2021-07-31', exclude_states=[], inlude_states=[]):
    ...
    for start in pd.date_range(date_from, date_to, freq='MS'):
        ...
        # Combine monthly data (pd.concat replaces deprecated DataFrame.append)
        race_details = pd.concat([race_details, month_race_details], ignore_index=True)
        dog_results = pd.concat([dog_results, month_dog_results], ignore_index=True)
